    ...
    def generate(
        ...
        Returns:
            (paths, behaviors, base_behavior)
             paths:      (B, num_paths, horizon)  in original (unscaled) returns space
             behaviors:  (B, num_paths, behavior_dim)
             base_behavior: (B, behavior_dim)
        """
        paths_scaled, B_agent, B0 = self.scheduler.sample(
            self, short_seq, mid_seq, long_seq,
            num_paths=num_paths,
            clip_denoised=clip_denoised,
        )
        # Unscale to original return space
        paths = paths_scaled / self.config.target_scale
        return paths, B_agent, B0
